...
import { getPlaceholderImage } from "@/lib/blue";
...
const Template = async ({
	...
	const imageWithPlaceholder = await Promise.all(
		...
		data?.gallery?.map(async (image) => {
			const imageWithPlaceholder = await getPlaceholderImage(image.imageUrl);
							...
							<>
								{(children.isRendered = true) && data?.gallerySingle && (
									<Gallery
										...
										className="mt-0 mb-12 mr-12 sm:mr-24 md:mr-10 lg:mr-32"
									>
										<a
